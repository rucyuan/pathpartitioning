...
class StartingVertexGenerator() extends Serializable {
  ...
  def generateStartingVertex(vertex: RDD[(Int, Boolean)], edge: RDD[(Int, Int)]): RDD[(Int, Set[Int])] = {
    ...
    breakable {
      ...
      while (true) {
        out = out.leftOuterJoin(edge.join(out.filter(t => t._2._3))
            .map(t => (t._2._1, (t._2._2._1, t._2._2._2)))
        ...
        .setName("out"+num)
        num += 1
        val numActives = out.filter(t => t._2._3).count()
        println(numActives)
        if ( numActives == 0) break
      }
    }
    
    /*output = out.mapValues{ case (src, m, _) => {
      (src, m)
    }}.setName("output").partitionBy(out.partitioner.get)*/
    out.persist(StorageLevel.MEMORY_AND_DISK)
    var ou = out.mapValues(t => if (t._1.isEmpty) (Set[Int](t._2), true) else (t._1, false))
    .setName("out"+num)
    num += 1
    ou.persist(StorageLevel.MEMORY_AND_DISK)
    breakable {
...
def maintainStartingVertex(edge: RDD[(Int, Int)], dedges: RDD[(Boolean, (Int, Int))]) : RDD[(Int, Set[Int])] = {
    ...
    .reduceByKey((a, b) => a && b).partitionBy(edge.partitioner.get).cache()
    ...
    .partitionBy(fout.partitioner.get).cache()
     vS
  }
}