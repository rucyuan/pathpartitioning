...
class StartingVertexGeneratorBackup(vertex: RDD[(Int, Boolean)], edge: RDD[(Int, Int)]) extends Serializable {
  ...
  */
  ...
  def maintainStartingVertex(edge: RDD[(Int, Int)], dedges: RDD[(Boolean, (Int, Int))]) : RDD[(Int, Set[Int])] = {
    ...
    dedges.filter{ t => t._1 }.map(t => t._2._2)
    .collect().filter(x => startVertice.contains(x))
    .foreach(println)
    
    startVertice = startVertices.filter(t => t._2).map(t => t._1).collect().toSet
